...
export default defineComponent({
  ...
  methods: {
    setOutputPathFromStorage() {
      if (this.appStore.prevOutputPath) {
        this.outputFilePath = this.appStore.prevOutputPath;
      }
    },
    handleBadExtension() {
    ...
    handleFilesLoaded(fileObjects: object[]) {
      ...
      if (list.length === 0) {
        ...
        this.setSuccessToastMsg(list.length);
    ...
    async setOutputPath() {
      ...
      if (result != null) {
        ...
        this.appStore.setPrevOutputPath(result);
      }
    },
    ...
    anyPrevExists(): boolean {
      for (const file of this.files) {
        const newFile = removeExtension(file.name) + " Prev" + this.outputFileExtension;
          ...
          if (currentFileIndex < this.files.length) {
            ...
            if (childProcess) { // Check if childProcess is not null
              ...
              childProcess.on('error', (err: any) => {
                ...
                currentFileIndex++;
                this.toast = {
                  message: 'Preview generation failed: ' + (err?.message || 'FFmpeg error'),
                  kind: 'alert-error',
                  timeout: 5000
                };
                this.$emit('toggle-toast', this.toast);
                processNextFile(); // Continue with remaining files
              });
            } else {
              console.error('Failed to spawn FFMpeg process.');
              currentFileIndex++;
              this.toast = {
                message: 'Preview generation failed: could not start FFmpeg.',
                kind: 'alert-error',
                timeout: 5000
              };
              this.$emit('toggle-toast', this.toast);
